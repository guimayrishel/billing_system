        ...
        "order": []
            ...
            $("#add").validate({
                ...
                submitHandler: function(form) {
                    var form_action = $("#add").attr("action");
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            var timeCalled = (res.data.time_called && res.data.time_called !== 'none') 
                            ? formatTime(res.data.time_called) 
                            : 'N/A';
                            ...
                            var crud = '<tr id="'+res.data.id+'">';
                                ...
                                crud += '<td>'+(res.data.time_arrived && res.data.time_arrived !== 'none'
                                                ...
                                                : '<a class="btn-warning p-1 text-white" style="border-radius: 5px;"><b>Waiting</b></a>')+'</td>';
                                crud +='<td> '+acceptButton+'</td>'                
                                crud += '</tr>';         
                            $('#tabledata tbody').prepend(crud);
                            $('#add')[0].reset();
                            $('#addModal').modal('hide');
                        },
                            error: function (data) {
            ...
            $("#update").validate({
                ...
                submitHandler: function(form) {
                    var form_action = $("#update").attr("action");
                    var formData = new FormData(form);
                    $.ajax({
                        data: formData,
                        url: form_action,
                        ...
                        success: function (res) {
                            ...
                            $('#tabledata tbody #'+ res.data.id).html(crud);
                            $('#update')[0].reset();
